...
\node at ([yshift=0cm] user.north west) [color=white, anchor=north west] {\textbf{Kernel space}};
%\node at ([yshift=-.2cm] kernel.north) [color=white, anchor=north] {\textbf{Kernel Space}};

\node (driver) at ([xshift=.25cm,yshift=1cm] user.south) [anchor=south, color=white, fill=TUMBlue, rounded corners=.1ex, minimum width=3cm, minimum height = .5cm] {\textbf{Driver}};
...
\node (app) at ([yshift=1cm] dpdk.north) [anchor=south, color=white, fill=TUMBlue, rounded corners=.1ex, minimum width=1.2cm, minimum height = .5cm] {\textbf{Application}};
...
\draw[ultra thick, color=TUMOrange] (app.south) to (dpdk);
